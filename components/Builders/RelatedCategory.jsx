...
export default function RelatedCategory({ productList, rowCount, scroll_button, scroll_id, productBoxView }) {
            ...
            return (
              <li onClick={() => router.push('/' + item.route)}  key={index} className={`${rowCount ? rowCount : 'flex-[0_0_calc(25%_-_8px)]'} ${(productBoxView && productBoxView == 'List View') ? 'md:flex-[0_0_calc(100%_-_0px)] flex items-center mb-[5px] rounded-[5px] md:border-[1px] md:border-slate-200 p-[5px]' : (scroll_button ? 'md:flex-[0_0_calc(60%_-_0px)] rounded-[5px] border-[1px] border-slate-200': 'md:flex-[0_0_calc(50%_-_0px)] md:border-b-[1px] md:odd:border-r-[1px]')} lg:border-[1px] lg:border-slate-200 lg:rounded-[5px] relative bg-[#fff] cursor-pointer`} >
                <div className={`flex cursor-pointer items-center justify-center lg:h-[160px] md:h-[140px] relative ${(productBoxView && productBoxView == 'List View') ? '' : 'p-[10px] '}`}>
                  ...
                  ></Image> */}
                  <ImageLoader  height={150} width={200} style={`lg:h-[150px] md:h-[125px] object-contain`} src={item.mobile_image} title={item.item ? item.item : 's'}  ></ImageLoader>
                 
                </div>
                <h3  className='text-[14px] cursor-pointer p-[10px] font-semibold line-clamp-2 capitalize text-center'>{item.category_name}</h3>
              </li>
            )
           })}
          </ul>
