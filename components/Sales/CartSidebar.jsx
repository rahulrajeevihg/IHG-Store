...
import { setCartItems, resetCart, updateItemQty, removeItem } from '@/redux/slice/cartSettings';
...
export default function CartSidebar({ onClose, onQuotationCreated }) {
  ...
  const exportCSV = () => {
    const rows = [
      ['Item Code', 'Item Name', 'Qty', 'Rate (AED)', 'Total (AED)'],
      ...cartItems.map((item) => [
        item.item_code,
  ...
  const toggleRecent = () => {
    ...
    setRecentOpen(next);
    if (next && !recentQuotations.length) loadRecentQuotations();
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-[#e5e7eb] text-[#111]">
      ...
      <div className="flex-1 overflow-y-auto px-[14px]" style={{ maxHeight: 'calc(100vh - 460px)' }}>
        {cartItems.length === 0 ? (
          ...
          <div className="divide-y divide-[#f3f4f6]">
            {cartItems.map((item) => (
              <CartItemRow key={item.cart_id || item.item_code} item={item} onSync={syncCart} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="px-[14px] pb-[14px] border-t border-[#e5e7eb] pt-[12px] space-y-[8px]">
        ...
        {/* Recent quotations toggle */}
        <button
          onClick={toggleRecent}
          className="w-full text-center text-[10px] text-[#9ca3af] hover:text-[#111] py-[4px] underline-offset-2 hover:underline"
        >
          {recentOpen ? 'Hide' : 'View'} Recent Quotations
        </button>

        {recentOpen && (
          <div className="border border-[#e5e7eb] rounded-[4px] overflow-hidden">
            <div className="px-[10px] py-[6px] bg-[#f9fafb] border-b border-[#e5e7eb] text-[10px] font-semibold uppercase tracking-[0.1em] text-[#6b7280]">
              Recent Quotations
            </div>
            {recentLoading ? (
              ...
              <p className="px-[10px] py-[10px] text-[11px] text-[#9ca3af]">No quotations found</p>
            ) : (
              <ul className="max-h-[160px] overflow-y-auto divide-y divide-[#f3f4f6]">
                {recentQuotations.map((q) => (
                  <li key={q.name} className="px-[10px] py-[7px] hover:bg-[#f9fafb]">
                    <a
                      href={`http://${typeof window !== 'undefined' ? window.location.hostname.replace(/:\d+$/, '') : ''}:8000/app/quotation/${q.name}`}
                      target="_blank"
                      rel="noreferrer"
                      className="block"
                    >
                      <div className="flex items-center justify-between gap-[6px]">
                        <span className="text-[11px] font-semibold text-[#111] font-mono">{q.name}</span>
                        <span className={`text-[9px] font-bold uppercase px-[4px] py-[1px] ${
                          q.status === 'Draft' ? 'bg-[#fef9c3] text-[#92400e]' :
                          q.status === 'Submitted' ? 'bg-[#dcfce7] text-[#15803d]' :
                          'bg-[#f3f4f6] text-[#6b7280]'
                        }`}>{q.status}</span>
                      </div>
                      <p className="text-[10px] text-[#9ca3af] mt-[1px] truncate">{q.customer_name}</p>
                      <p className="text-[10px] text-[#6b7280] font-medium">AED {parseFloat(q.grand_total || 0).toFixed(2)}</p>
                    </a>
                  </li>
                ))}
              </ul>
...
/* ── Cart Item Row ── */
function CartItemRow({ item, onSync }) {
  const dispatch = useDispatch();
  ...
  return (
    <div className="flex items-start gap-[8px] py-[10px]">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] font-bold text-[#111] mt-[3px]">AED {lineTotal.toFixed(2)}</p>
      </div>
...
/* ── Quotation Preview Modal ── */
function QuotationPreviewModal({ cartItems, opportunity, grandTotal, isCreating, onConfirm, onClose }) {
  return (
